...
if (isset ( $_POST ['query'] )){
    ...
    $option_padding = 1;
	if($brand_selected>0){
		$brand_selected_serial = $brands[$brand_selected-$option_padding]["$column_brand_serial"];
	}
	else{
		$brand_selected_serial = 0;
	}
	...
	if($model_selected>0){
		$model_selected_serial = $models[$model_selected-$option_padding]["$column_model_serial"];
	}
	else{
		$model_selected_serial = 0;
	}
	...
	if($province_selected>0){
		$province_selected_serial = $provinces[$province_selected-$option_padding]["$column_province_serial"];
	}
	else{
		$province_selected_serial = 0;
	}
	...
	if($city_selected>0){
		$city_selected_serial = $cities[$city_selected-$option_padding]["$column_city_serial"];
	}
	else{
		$city_selected_serial = 0;
	}
...
//[option_text,serial_index,option_value]
echo 'brand_selectArr[0] = ["全部品牌","0","0"];';
//[option_text,serial_index, parent_serial, option_value]
echo 'model_selectArr[0] = ["全部机型","0","0","0"];';


for($i=0;$i<count($brands);$i++){
	echo 'brand_selectArr['.($i+$option_padding).'] = ["'.$brands[$i]["$column_brand"].'","'.$brands[$i]["$column_brand_serial"].'","'.($i+$option_padding).'"];';
	echo 'model_selectArr['.($i+$option_padding).'] = ["全部机型","0","'.$brands[$i]["$column_brand_serial"].'","0"];';
	$d2++;
}

for($j=0;$j<count($models);$j++){
	echo 'model_selectArr['.($i+$option_padding+$j).'] = ["'.$models[$j]["$column_model"].'","'.$models[$j]["$column_model_serial"].'","'.$models[$j]["$column_brand_serial"].'","'.($option_padding+$j).'"];';
}

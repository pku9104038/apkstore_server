...
$brands[0]["$column_brand"]="Namo";
$brands[0]["$column_brand_serial"]=1;

require_once '../utilities/class.modelmanager.php';
$modelManager = new ModelManager();
$models = $modelManager->getModelByBrand(1);
...
for($i=0;$i<count($brands);$i++){
	echo 'brand_selectArr['.($i).'] = ["'.$brands[$i]["$column_brand"].'","'.$brands[$i]["$column_brand_serial"].'","'.($i+1).'"];';
	echo 'model_selectArr['.($i).'] = ["全部机型","0","'.$brands[$i]["$column_brand_serial"].'","0"];';
}

for($j=0;$j<count($models);$j++){
	echo 'model_selectArr['.($i+$j).'] = ["'.$models[$j]["$column_model"].'","'.$models[$j]["$column_model_serial"].'","'.$models[$j]["$column_brand_serial"].'","'.(1+$j).'"];';
}

...
if (isset ( $_POST ['submit'] )) 
{
    ...
    if( $mgr->checkCustomer($customer)){
        $err_customer = "该客户已经存在！";
        $output_form = TRUE;
    }
    
    
    if (!$output_form){
        if($mgr->addCustomer($customer, $type_id,$contact, $email, $phone, $notes)){
            $err_msg = '新客户"'.$customer.'"添加成功！';
        }
        else{
            $err_msg = '新客户"'.$customer.'"添加失败！';
            $output_form = TRUE;
        }
    }
					...
					<div align="center">
						<table>
							...
							<tr>
								...
								<td >
									<input id="customer" name="customer" type="text" value="<?php echo $customer; ?>" />

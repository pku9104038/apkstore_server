<?php
/**
 * AppActiveManager - ApkStore app_active counts management class
 * NOTE: 
 * Dependencies:
 *     'class.log.php' 
...
class AppActiveManager{
	...
	private $table                  = DatabaseProxy::DB_TABLE_APP_ACTIVES;
	
	private $column_serial          = DatabaseProxy::DB_COLUMN_APP_ACTIVE_SERIAL;
	private $column_device	     	= DatabaseProxy::DB_COLUMN_APP_ACTIVE_DEV_SERIAL;
	private $column_date 			= DatabaseProxy::DB_COLUMN_APP_ACTIVE_DATE;
	private $column_counts       	= DatabaseProxy::DB_COLUMN_APP_ACTIVE_COUNTS;
	private $column_stamp         	= DatabaseProxy::DB_COLUMN_APP_ACTIVE_REPORT_STAMP;
	...
	public function updateRecord($devsn, $date, $counts){
		$result = FALSE;
	
		if($this->pdo != null){
	
			$count = $this->getRecordCounts($devsn, $date)+$counts;
			$sql = "UPDATE $this->table".
					" SET $this->column_counts = $count".
					" WHERE $this->column_device = $devsn AND $this->column_date='$date'";
	...
	public function getRecordCounts($devsn, $date){
		$result = 0;
		if($this->pdo != null){
			$sql = "SELECT * FROM $this->table WHERE $this->column_device = $devsn AND $this->column_date='$date'";
	...
	public function fetchDeviceAppActiveByPeriod($period_array,$cur_page, $limit)
	{
		$column_name = "name";
		$column_value = "value";
	...
	public function addRecords($array){
		
		if($this->pdo != null && $array){
		
			foreach ($array as $record){
				$devsn = $record["$this->column_device"];
				$date = $record["$this->column_date"];
				$counts = $record["$this->column_counts"];
				if ($this->isNewRecord($devsn,$date)) {
					$this->addRecord($devsn, $date, $counts);
				}
				else {
					$this->updateRecord($devsn, $date, $counts);
				}
			}
		}		
	} 
